...
// virtual
void TarjanVishkin::getBiCC(const Graph_test &g, vector< set<size_t> > &bicc)
{
	...
	cout << "Number of biconnected components: " << bicc.size() << endl;

	// print the bicc
	// for(size_t i = 0; i < bicc.size(); i++)
	// {
	// 	cout << "Biconnected component " << i << ": ";
	// 	for(set<size_t>::iterator it = bicc[i].begin(); it != bicc[i].end(); it++)
	// 		cout << *it << " ";
	// 	cout << endl;
	// }
}
...
{
	const vector<Edge_test> &edges = t.edges();
...
void TarjanVishkin::auxiliaryGraph(const Graph_test &g)
{
