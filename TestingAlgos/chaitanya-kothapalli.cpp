...
// virtual
void ChaitanyaKothapalli::getBiCC(const Graph_test &g, vector< set<size_t> > &bicc)
{
	Graph_test t, nt;
	vector<size_t> parent, level;
	vector< vector<size_t> > components;
	vector<Edge_test> bridges;
...
void ChaitanyaKothapalli::removeBridges(const Graph_test &g, const Graph_test &t, const Graph_test &nt, vector<size_t> &parent, vector<size_t> &level, vector< vector<size_t> > &components, vector<Edge_test> *bridges)
{
	Graph_test gPrime(g);
	...
	for(size_t i = 1; i < g.V(); i++)
	{
		if(!marked[i])
		{
			...
			if(bridges)
				bridges->push_back(Edge_test(i, parent[i]));
		}
	}
...
void ChaitanyaKothapalli::auxiliaryGraph(const Graph_test &g, const Graph_test &nt, const vector<size_t> &parent, const vector<size_t> &level, const vector<size_t> &component, Graph_test &aux, vector<size_t> &alias)
{
